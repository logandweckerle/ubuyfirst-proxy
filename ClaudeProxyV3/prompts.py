...
- Sterling melt rate: ${sterling_rate:.2f}/gram (USE THIS EXACT VALUE!)
...
=== CRITICAL: STONE DEDUCTIONS ===
ALWAYS DEDUCT STONE WEIGHT FROM TOTAL BEFORE CALCULATING MELT VALUE!

** LARGE CABOCHON STONES (coral, jade, turquoise, opal, amber) **
These are HEAVY and must be deducted:
| Size | Deduction |
| Small (under 10mm) | 1-2g |
| Medium (10-15mm) | 2-4g |
| Large (15-25mm) | 4-6g |
| Very Large (25mm+) | 6-10g+ |

For the coral ring in images: That's a LARGE cabochon (~20mm), deduct 4-6g from total weight!

** STANDARD GEMSTONES **
| Stone Type | Per Stone Deduction |
| Accent/chip/melee diamonds | 0.1-0.2g |
| Small gemstone (3-5mm) | 0.3-0.5g |
| Medium gemstone (5-8mm) | 0.5-1g |
| Large gemstone (8mm+) | 1-2g |

** EXAMPLE CALCULATION WITH STONES **
Ring with large coral cabochon:
- Total weight stated: 8g
- Coral cabochon (~20mm): -5g deduction
- GOLD WEIGHT: 8g - 5g = 3g
- Melt value: 3g x $81/g = $243 (NOT $648!)

ALWAYS show stone deduction in your reasoning!
...
=== REASONING FORMAT (REQUIRED) ===
"DETECTION: [karat], [item type], [weight], [stones if any] | STONES: [deduction amount and why, or 'None'] | CALC: [gold weight after deduction]g x $[rate] = $[melt], maxBuy (x0.90) = $[max], sellPrice (x0.96) = $[sell], Price $[price] | PROFIT: $[diff] | DECISION: [BUY/PASS] [rationale]"

Example with stones:
"DETECTION: 14K ring, 8.01g total, large coral cabochon ~20mm | STONES: -5g for coral cabochon | CALC: 3g gold x $81.27 = $244 melt, maxBuy = $220, sellPrice = $234, Price $470 | PROFIT: -$236 | DECISION: PASS negative margin after stone deduction"
...
- weight: TOTAL weight stated like "8.01g" or "NA"
- stoneDeduction: weight deducted for stones like "5g coral" or "0" or "NA"
- goldweight: weight AFTER stone deduction = weight - stoneDeduction
- meltvalue: goldweight x karat rate (NOT total weight!)
...
- pricepergram: TotalPrice divided by goldweight
- confidence: "High"/"Medium"/"Low"
- fakerisk: "High"/"Medium"/"Low"/"NA"
- reasoning: Calculation summary with stone deductions shown
