...
# RACE TEST - Compare API speed vs uBuyFirst

# ============================================================

# Store detected items for race comparison
...
# AUTOMATIC RACE COMPARISON - uBuyFirst vs API

# ============================================================

# Track items from both sources with timestamps

RACE_ITEMS = {}  # item_id -> {"ubuyfirst_time": datetime, "api_time": datetime, "title": str, "price": float}

RACE_RUNNING = False

RACE_STATS = {"ubuyfirst_wins": 0, "api_wins": 0, "ties": 0, "total": 0}

RACE_LOG = []  # List of comparison results
...
# Live feed tracking - separate streams for visual comparison

RACE_FEED_UBUYFIRST = []  # Last 30 items from uBuyFirst
...
@app.get("/api/source-comparison/races")
...
@app.get("/api/source-comparison/reset")
...
# Focused Gold Race - matches exact uBuyFirst "New Gold Search" settings

# All 26 keywords from uBuyFirst "New Gold Search" export
...
RACE_SEEN_IDS = set()  # Items seen before race started

RACE_KEYWORD_INDEX = 0



@app.get("/ebay/race/gold")
...
# ============== FULL RACE - ALL SEARCHES ==============



@app.get("/ebay/race/full")
...
async def keepa_stats():
    ...
    monitor_client = get_client()  # From keepa_tracker_v2.py
    global _keepa_client

    client = monitor_client or _keepa_client
...
async def keepa_check_deals():
    ...
    for drop in matches:
        ...
        await keepa_send_discord_alert(drop)
...
async def keepa_test_discord():
    ...
    try:
        ...
        await keepa_send_discord_alert(test_drop)
