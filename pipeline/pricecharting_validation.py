...
def validate_tcg_lego_result(result: dict, pc_result: dict, total_price: float, category: str, title: str = "") -> dict:
    ...
    if pc_result and pc_result.get('is_graded'):
        ...
        # PSA 10 vintage cards should always be RESEARCH (high fake risk)
        elif grader == 'PSA' and grade == 10:
            # Check if likely vintage (WOTC era keywords)
            title_lower = title.lower()
            vintage_keywords = ['base set', '1st edition', 'shadowless', 'jungle', 'fossil',
                              'team rocket', 'neo', 'legendary collection', 'expedition', 'aquapolis', 'skyridge']
            is_vintage = any(kw in title_lower for kw in vintage_keywords)

            if is_vintage and ai_rec == 'BUY':
                result['Recommendation'] = 'RESEARCH'
                result['reasoning'] = result.get('reasoning', '') + " | SERVER: PSA 10 vintage = verify cert number before buying"
                logger.info(f"[TCG-GRADED] Override: BUY->RESEARCH (PSA 10 vintage)")

        # Good margin + verified = BUY
        elif margin >= 30 and confidence in ['High', 'Medium'] and ai_rec == 'RESEARCH':
